...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange(event) {
    ...
    if (!this.currentVariant) {
      ...
      this.updateContinueSellingWhenOutOfStockMessage()
    }
  }

  updateVariantDescription() {
    const selectedVariant = `${this.dataset.section}-${this.currentVariant.id}`
    const variantsDescriptionParent = document.getElementById(
      "variants-description"
    )
    const descriptionsItems = variantsDescriptionParent.querySelectorAll(
      `.variant-description-item`
    )
    const activeDescriptionsItem = variantsDescriptionParent.querySelector(
      `[data-variant-id="${selectedVariant}"]`
    )
    console.log({ descriptionsItems })

    descriptionsItems.forEach((descriptionItem) => {
      descriptionItem.classList.add("hide-item-from-view")
    })
    activeDescriptionsItem.classList.remove("hide-item-from-view")

    // const variantDescriptionElement = document.getElementById(
    //   "variant-description"
    // )

    // variantDescriptionElement.innerText =
    //   allVariantsDescription[this.currentVariant.id]
  }

  updateContinueSellingWhenOutOfStockMessage() {
    const selectedVariant = `${this.dataset.section}-${this.currentVariant.id}`
    const wrapperElement = document.getElementById(
      "continue-selling-when-out-of-stock-message"
    )
    const messageItems = wrapperElement.querySelectorAll(
      `.variant-continue-selling-message`
    )
    const activeDescriptionsItem = wrapperElement.querySelector(
      `[data-variant-id="${selectedVariant}"]`
    )

    messageItems.forEach((descriptionItem) => {
      descriptionItem.classList.add("hide-item-from-view")
    })
    activeDescriptionsItem.classList.remove("hide-item-from-view")
  ...
  updateVariantMedia() {
    const selectedVariant = `${this.dataset.section}-${this.currentVariant.id}`
    const mediaRelatedToSelectedVariant = document.querySelectorAll(
      `[data-variant-id="${selectedVariant}"]`
    )
    const mediaRelatedToCurrentSection = document.querySelectorAll(
      `li[data-section-id="${this.dataset.section}"]`
    )

    mediaRelatedToCurrentSection.forEach((mediaItem) => {
      mediaItem.classList.add("hide-item-from-view")
    })
    mediaRelatedToSelectedVariant.forEach((mediaItem) => {
      mediaItem.classList.remove("hide-item-from-view")
    })
  }
